...
When you run `npm start`, a server will be launched in your terminal for development. You can then open [http://localhost:3030](http://localhost:3030) to access the server and see your app.
...
Run `npm start` to launch the application. If you start browsing at [https://localhost:3030](https://localhost:3030), by default you will be navigated to the home page. Here, notice that route is `"/"`, so the [`<HomePage />`](https://github.com/react-boilerplate/react-boilerplate/blob/master/app/containers/HomePage/index.js) container will be mounted. It is responsible for rendering a form with a textbox and a list of repositories.
